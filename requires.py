...
class MySQLRouterRequires(Endpoint, metaclass=MySQLRouterAutoAccessors):
    ...
    def set_or_clear_available(self):
        ...
        if self.ssl_data_complete():
            ...
            clear_flag(self.expand_name('{relation_name}.available.ssl'))
    ...
    def departed(self):
        ...
        if last_unit:
            # Bug #1972883
            for relation in self.relations:
                relation.to_publish_raw['prefixes'] = []
    ...
    def _set_local(self, key, value):
        unitdata.kv().set(self.key.format(key), value)

    def _get_local(self, key):
        return unitdata.kv().get(self.key.format(key))
